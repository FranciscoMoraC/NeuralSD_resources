...
def load_brca_bin(train = False):
    if not train:
        ...
        path = "../../data/brca_n_train.csv"
        data = pd.read_csv(path)
        labels = (data["Target"].to_numpy() == 1).astype(int)
        data = data.drop(columns = ["Target"])
        translator = data.columns
        data = data.to_numpy().astype(np.float32)
    return data, labels, translator

def load_facebook(train = False):
    if train:
        path = "../../data/facebook_train.csv"
    else:
        path = "../../data/facebook.csv"
    data = pd.read_csv(path)
    labels = (data["target"].to_numpy() == "government").astype(int)
    data = data.drop(columns = ["target"])
    translator = data.columns
    data = data.to_numpy().astype(np.float32)
    return data, labels, translator

def load_synthetic(train = False):
    if train:
        path = "../../data/synthetic3_train.csv"
    else:
        path = "../../data/synthetic3_all.csv"
    df = pd.read_csv(path)
    labels = (df["Target"].to_numpy() == 1).astype(int)
    data = df.drop(columns = ["Target"])
    translator = data.columns
    data = data.to_numpy().astype(np.float32)
    return data, labels, translator

def load_synthetic_polluted(train = False):
    if train:
        path = "../../data/synthetic3-5_train.csv"
    else:
        path = "../../data/synthetic3-5_all.csv"
    df = pd.read_csv(path)
    labels = (df["Target"].to_numpy() == 1).astype(int)
    data = df.drop(columns = ["Target"])
    translator = data.columns
