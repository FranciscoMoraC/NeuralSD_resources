...
def get_model(model, write,database = None):
    ...
    if model == "IDSD":
        ...
        return SDIGA(max_generation=params["max_generation"], write_results_in_file=write, file_path=output_file, population_size = params["population_size"],
                        crossover_prob = params["crossover_prob"], mutation_prob = params["mutation_prob"], confidence_weight = params["confidence_weight"], support_weight = params["support_weight"],
                        min_confidence = 0.1, max_pattern_size = params["max_size"], num_subgroups = params["num_subgroups"]
                   ), id
    else:
...
def covered_instances(model, database):
    ...
    return compute_covered_instances(model, id, df, target, database, remove_file=False)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--population_size', type=int, help='Population size for SDIGA')
    parser.add_argument('--max_generation', type=int, help='Maximum number of generations for SDIGA')
    parser.add_argument('--crossover_prob', type=float, help='Crossover probability for SDIGA')
    parser.add_argument('--mutation_prob', type=float, help='Mutation probability for SDIGA')
    parser.add_argument('--confidence_weight', type=float, help='Confidence weight for SDIGA')
    parser.add_argument('--support_weight', type=float, help='Support weight for SDIGA')
    ...
    if args.max_generation is not None:
        params["max_generation"] = args.max_generation
    if args.crossover_prob is not None:
        params["crossover_prob"] = args.crossover_prob
    if args.mutation_prob is not None:
        params["mutation_prob"] = args.mutation_prob
    if args.confidence_weight is not None:
        params["confidence_weight"] = args.confidence_weight
    if args.support_weight is not None:
        params["support_weight"] = args.support_weight
